...
fn phase_col_width_uniform_short_phases_clamped_to_4() {
    ...
    let items_ref: Vec<&spacetop_core::domain::Entity> = vec![&run_item];
...
fn phase_col_width_mixed_phases_fits_longest() {
    ...
    let items_ref: Vec<&spacetop_core::domain::Entity> = items_data.iter().collect();
    let pcw = items_ref
        .iter()
        .map(|i| i.status.chars().count())
        .max()
...
fn phase_col_width_long_phase_name_clamped_at_12() {
    ...
    let items_ref: Vec<&spacetop_core::domain::Entity> = vec![&long_item];
    let pcw = items_ref
        .iter()
        .map(|i| i.status.chars().count())
        .max()
