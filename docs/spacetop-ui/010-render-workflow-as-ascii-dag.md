...
- DONE: Horizontally center the DAG within the pane: compute the actual rendered width of the chain (single-row and multi-row layouts) and pad both sides so the chain sits in the middle of `inner_width`. The feedback-arc row's corners/segments/arrowhead must shift with the chain so they remain column-aligned with their source/target stages. Apply to both single-row (e.g. spacetop-dev 5 stages) and multi-row (e.g. dataagentbench/research 12 stages at narrow widths) layouts.
  Refactored `render_dag` (src/ui/graph.rs) into a two-pass renderer: first builds every DAG row (chain rows, inter-row connectors, feedback-arc rows, cross-row fallback annotations) into a `RowContent { spans, width }` vec WITHOUT padding; second pass computes `chain_width = max(row_widths).min(inner_width)`, `slack = inner_width - chain_width`, `left_pad = slack / 2`, then frames every line with the same `left_pad` and right-pads to `inner_width`. Because every line — chain, connector, arc — shares the same `left_pad`, the arc-row `↑` arrowhead and `│` source-column glyphs stay column-aligned with the corresponding stage spans on the chain row above. Dropped the now-dead `dag_arc_line` helper; `horizontal_margins` / `usable_inner_width` survive (still used by Narrow / VeryNarrow tiers, which are out of scope per cycle-2 feedback).
- DONE: Add a test on the spacetop-dev fixture (5 stages, fits in single row at typical widths) asserting (a) the leftmost rendered DAG column is approximately `(inner_width - chain_width) / 2` (within a one-column rounding tolerance), (b) the rightmost rendered column is approximately the same distance from the right edge, and (c) the arc row's source-column glyph still column-aligns with the source stage span and the target-column arrowhead with the target stage span. Add a sibling test on the 12-stage research fixture at a narrow width that exercises the multi-row layout and asserts the same centering property per row.
  `dag_chain_is_horizontally_centered_on_spacetop_dev` (109x10): computes `chain_width` from non-space bounds on the `►`-bearing chain row, asserts `left_gap ≈ slack/2`, `right_gap ≈ slack - left_gap`, and `left_gap` agrees with `right_gap` within one column. It then locates the arrow row (`↑` + `│` + `reject`), derives target_col / source_col, and walks the chain row to find the `implement` and `review` stage spans — asserting target_col falls within the `implement` span and source_col within the `review` span. `dag_multi_row_chain_is_horizontally_centered_on_research_fixture` (100x10): walks every DAG row between the first and last `►`-bearing rows, computes the DAG block bounds, and asserts the block is centered in inner_width AND every row's leftmost column equals the block_left within one column (the captain's spec: uniform left_pad across all DAG rows, not per-row centering — per-row centering would force connectors and arc rows to drift relative to the chain spans they reference).
- DONE: make lint clean and full cargo test green; the cycle-1 multi-row + drawn-arc tests are preserved or updated in lockstep with the new centering math.
  `make lint` (`-D warnings`) clean. `cargo test` 288 lib + 4 + 10 integration = 302 tests pass (3 notify-backed watcher tests ignored as before). All 8 prior DAG tests (cycle-0 + cycle-1) — including `dag_drawn_feedback_arc_is_fully_connected_on_spacetop_dev` and `dag_multi_row_wraps_with_drawn_connectors_on_research_fixture` — pass unchanged: the centering refactor shifts every line uniformly, so column-relative assertions still hold. Two new cycle-2 tests added.

### Summary

`render_dag` is now centered: rather than left-aligning at a fixed `left_margin` derived from `usable_width`, it measures every DAG row's actual content width, takes the max as `chain_width`, and splits `inner_width - chain_width` evenly between left and right pads. The same `left_pad` is applied to chain rows, inter-row connectors, feedback-arc rows, and cross-row fallback annotations so source-column `│` and target-column `↑` glyphs stay aligned with their corresponding stage spans on the chain row. Cycle-1's multi-row layout and fully-connected drawn-arc work are preserved unchanged. Two new tests pin the centering contract on the spacetop-dev (single-row) and research 12-stage (multi-row) fixtures; lint and the full test suite remain green.
