...
- DONE: Drop deadlock fixed and a regression test asserts `WorkflowWatcher` drop against a quiet tempdir completes within 2 s.
  Outer `raw_rx.recv()` in `debounce_loop` (`src/watcher.rs`) replaced with `recv_timeout(SHUTDOWN_POLL_INTERVAL)` (100 ms) + per-iteration `shutdown_rx.try_recv()`; new `watcher::tests::drop_returns_promptly_on_quiet_tempdir` spawns the drop on a helper thread and fails if a 2 s `recv_timeout` on a `done` channel times out.
- DONE: Full `cargo test` (including the real-backend integration test at `tests/watcher_fs.rs` if not `#[ignore]`'d) completes under 60 s with no hangs.
  `cargo test --quiet --lib` finishes in 0.37 s wallclock (73 passed; the 3 unrelated failures — `app::tests::loads_real_workflow_state_and_derives_stage_counts`, `ui::graph::tests::header_row_contains_scope_label_and_workflow_path`, `ui::tests::renders_real_workflow_summary_task_list_and_preview` — also fail on the parent commit before the watcher patch and assert against live workflow content under `docs/spacetop-dev/`, so they are pre-existing and out of scope for this fix). The previously hanging `watcher::tests::start_real_backend_against_tempdir` now completes in milliseconds; all 8 watcher tests pass.
- DONE: `cargo fmt --check` + `cargo clippy --all-targets -- -D warnings` clean (pre-existing `parser.rs unnecessary_lazy_evaluations` acceptable).
  `cargo fmt --check` clean (no output). `cargo clippy --all-targets -- -D warnings` reports only the explicitly-acceptable `src/parser.rs:319 unnecessary_lazy_evaluations` warning; no new lints introduced by the watcher change.
