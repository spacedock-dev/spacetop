...
- DONE: Loader contract change per plan: `load_workflow_dir` collects per-entity parse failures into `WorkflowSnapshot::parse_errors` (Vec<EntityParseError>) instead of bailing on the first `?`. `parse_workflow_readme` (workflow `README.md`) stays strict and still returns `Err`. Downstream consumers (lib.rs::decide_app, src/app.rs, src/ui/*) thread the new field through. Cite the new `EntityParseError` struct fields and the updated call site.
  Added `EntityParseError { path, message, line, column }` and `WorkflowSnapshot.parse_errors` (`src/domain/mod.rs`); rewrote the per-item loop in `src/parser/snapshot.rs::load_workflow_dir` to classify-and-collect via `ParseError::is_per_entity_parse_failure()` and `ParseError::yaml_location()` (`src/parser.rs`); mirrored the same shape in `scan_worktrees`/`load_worktree_items` (`src/parser/worktree.rs`); `OverviewState` exposes `parse_errors()` and `selected_row()` returning a new `SelectedRow` enum, re-exported from `src/app.rs`.
- DONE: In-app surface per plan: synthetic 'broken' rows appended to the task list with the per-entity error rendered in the preview pane (file path, YAML error message, line/column when derivable, remediation hint about quoting colon values). Footer pill `⚠ N broken` shown when `parse_errors` is non-empty. Cite the src/ui/* render functions changed and add buffer-substring tests against TestBackend.
  `src/ui/list.rs::build_task_list_items` now appends `broken_list_item(...)` rows after the regular items; `src/ui/preview.rs::render_broken_entity_preview` renders the path, error, `line X, column Y`, and the pinned `BROKEN_ENTITY_HINT` constant; `src/ui/footer.rs::status_footer_hints` prepends a `⚠ N broken` pill (red foreground on PILL_BG). Four new TestBackend tests in `src/ui/tests/task_list.rs` cover the row, the preview pane, the pill, and the no-pill negative.
- DONE: Tests covering AC-1..AC-5 from the plan's test strategy (named in `## Stage Report: plan`): N valid + 1 malformed -> N items loaded + 1 parse_error; per-entity error surface populated with path/error/line/col; decide_app returns a launch decision (not Err) for the malformed-entity case; workflow-README malformed still produces a top-level Err + non-zero exit; the deliberate softening (all-entities-malformed loads empty snapshot + N broken rows) is exercised by a test that documents that decision. `make lint` clean and `cargo test` green; pre-existing parser/discovery tests unchanged or updated in lockstep.
  AC-1+AC-4: `src/parser/tests.rs` adds `load_workflow_dir_skips_malformed_entity_and_records_error`, `load_workflow_dir_all_malformed_entities_loads_empty_with_broken_rows`, and `load_workflow_dir_malformed_readme_still_fatal`. AC-2: `src/app/tests.rs::overview_state_exposes_parse_errors_from_snapshot` + UI tests above. AC-3: `tests/discovery_bypass.rs::decide_app_launches_overview_when_one_entity_has_malformed_frontmatter`. AC-4 integration: `decide_app_returns_err_when_workflow_readme_is_malformed`. Pre-existing `reload_retains_prior_snapshot_on_parse_error` was rewritten in lockstep as `reload_records_per_entity_parse_error_without_dropping_other_items` to reflect the deliberate softening. `make lint` clean; full suite: 278/278 lib + 10/10 discovery_bypass + 4/4 ui-tests.

### Summary

Implemented the planned classify-and-collect loader contract: per-entity parse failures (MalformedYaml, MissingFrontmatter, UnterminatedFrontmatter, MissingRequiredField, UnknownStatus) flow into `WorkflowSnapshot.parse_errors` rather than aborting the load; FS-level errors (ReadFile, ReadDirectory) stay fatal. The TUI now appends synthetic `⚠ broken: <file>` rows after the task list, renders the YAML error + `Hint: wrap values containing ':' …` (pinned string) in the preview pane, and shows a red `⚠ N broken` footer pill. AC-3 is verified end-to-end via `decide_app`, and AC-4 keeps a malformed workflow README fatal. The deliberate "all-entities-malformed → empty snapshot + N broken rows" softening is intentionally exercised by a test that documents the decision for the review gate.
