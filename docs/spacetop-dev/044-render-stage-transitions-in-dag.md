...
- DONE: Reviewer independently confirms each AC (AC-1 through AC-5) with concrete evidence from the implement diff and test output — cite file:line ranges and test names; do not rubber-stamp claims from the implement Stage Report.
  AC-1 verified by `transitions_block_is_parsed_into_definition`, `missing_transitions_block_leaves_empty_vec`, `transitions_block_with_labels_round_trips`, `parse_workflow_readme_research_fixture_terminal_edges` (src/parser/readme.rs:421-526) and the new `transitions: Vec<StageTransition>` field on `WorkflowDefinition` (src/domain/mod.rs:101-105, 131-152). AC-2/AC-3 verified by `dag_renders_inbound_edge_for_non_adjacent_terminal_predecessor`, `dag_renders_three_inbound_edges_for_rejected`, and `collect_extra_transitions_for_research_fixture_lists_all_non_adjacent_edges` (src/ui/graph/tests.rs:2378-2598) backed by `collect_extra_transitions` (src/ui/graph.rs:823-872) and the annotation emission at src/ui/graph.rs:624-650. AC-4 verified by `dag_omits_arcs_when_no_transitions_block` (src/ui/graph/tests.rs:2509-2554) plus `effective_transitions_synthesizes_linear_chain_when_absent` (src/domain/mod.rs:298-316) and continued passing of existing renderer tests. AC-5 verified — `make lint` clean (clippy `-D warnings` finishes with no diagnostics); `cargo test` shows 303 lib + 10 integration tests passing; `git diff main...HEAD` introduces no `#[allow(...)]`.
- DONE: Reviewer validates that the graph rendering for a dataagentbench-style research fixture now shows the four terminal stages connected to their declared predecessors — including all three sources for rejected — by inspecting graph test output or rendering the fixture directly.
  Verified via `dag_renders_inbound_edge_for_non_adjacent_terminal_predecessor` which asserts the rendered string contains `scoping → expanded`, `ideate → ideated`, `promote → done`; `dag_renders_three_inbound_edges_for_rejected` asserts exactly 3 `→ rejected` substrings with sources `review`, `smoke`, `analyze`. The fixture `research_workflow_definition()` (src/ui/graph/tests.rs:2381-2421) mirrors the dataagentbench research workflow's 12 states and 13 transitions verbatim.
- DONE: Reviewer issues a PASSED or REJECTED verdict; if REJECTED, list concrete fix asks pointing at file:line and the AC each finding maps to so the implement worker can act without further interpretation.
  Verdict below — PASSED.

### Summary

PASSED. All five acceptance criteria are met with concrete test evidence. The implementation deviates from the plan's sketched overhead-arc geometry by uniformly using the annotation-tail degradation pattern for both non-adjacent same-row and cross-row edges — this deviation is explicitly documented in the implement Stage Report and is consistent with what the ACs require (AC-2 asks for "visibly linked" inbound edges; AC-3 asks for the three sources to be represented; neither mandates arc geometry). Note for the captain's awareness: the inline chain row still draws stages in declaration order (so the chain `… promote ──▶ expanded ──▶ ideated ──▶ done ──▶ rejected` still appears as today), and the new annotation tails are additive truthful edges below it; this is acceptable given the ACs and plan, but a future polish item could be to suppress the misleading inline arrows between non-adjacent terminals if the captain wants the inline chain to also reflect the true topology. `make lint` is clean and the full test suite is green.
