...
- DONE: AC-5 PASS — `parser::tests::worktree_scan_handles_no_worktrees_missing_subdir_and_partial_overlap` (src/parser/tests.rs:850) exercises all three sub-cases the AC requires: (a) repository with no worktree directories registered — asserts `load_workflow_dir` returns `Ok`, the merged list equals the root-only baseline (one item `Alpha`), and `worktree_source.is_none()`; (b) a worktree directory exists but does not contain the workflow subdir (only `.worktrees/wt-1/unrelated`) — asserts no error, list still has one item `Beta` with `worktree_source.is_none()`, no spurious rows; (c) worktree contains the workflow dir but its task files (`delta.md`) do not overlap with the root (`gamma.md`) — asserts both `Gamma` (main-sourced, `worktree_source.is_none()`) and `Delta` (worktree-only, `worktree_source.is_some()`) appear with no errors. Test observed passing in the cargo test run recorded below.
